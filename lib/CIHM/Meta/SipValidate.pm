package CIHM::Meta::SipValidate;

use strict;
use Carp;
use CIHM::WIP;
use Log::Log4perl;
use CIHM::Meta::SipValidate::Worker;
...
CIHM::Meta::SipValidate - Validates a SIP within the TDR.


=head1 SYNOPSIS

    my $validate = CIHM::TDR::SipValidate->new($args);
...
sub new {
    ...
    if (ref($args) ne "HASH") {
        die "Argument to CIHM::TDR::SipValidate->new() not a hash\n";
    };
    $self->{args} = $args;
    ...
    if (! $self->wipmeta ) {
        die "No access to 'wipmeta' database\n";
    }
    Log::Log4perl->init_once("/etc/canadiana/tdr/log4perl.conf");
    $self->{logger} = Log::Log4perl::get_logger("CIHM::Meta::SipValidate");
...
sub validate {
    ...
    $self->log->info("SIP validate: conf=".$self->configpath." skip=".$self->skip. " limit=".$self->limit. " maxprocs=" . $self->maxprocs . " timelimit=".$self->{timelimit});

    my $pool = AnyEvent::Fork
        ->new
        ->require ("CIHM::Meta::SipValidate::Worker")
        ->AnyEvent::Fork::Pool::run 
        (
         "CIHM::Meta::SipValidate::Worker::validate",
         max        => $self->maxprocs,
         load       => 2,
...
sub getNextAIP {
    ...
    my $res = $self->wipmeta->get("/".$self->wipmeta->{database}."/_design/tdr/_view/sipvalidateq?reduce=false&limit=".$self->limit.$skipparam,{}, {deserializer => 'application/json'});
    if ($res->code == 200) {
        ...
        warn "_view/sipvalidateq GET return code: ".$res->code."\n"; 
    }
    return;
}
