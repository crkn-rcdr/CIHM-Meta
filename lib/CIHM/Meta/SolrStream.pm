...
sub getNextStream {
    ...
    my $res = $self->cosearch->get("/".$self->cosearch->database."/_changes?include_docs=true&since=$since&limit=".$self->limit,{}, {deserializer => 'application/json'});
    if ($res->code == 200) {
...
sub postSolrStream {
    ...
    my $res = $self->cosolr->post("solr/".$self->cosolrdb."/update",$stream);
...
sub getSince {
    ...
    my $since;
    $self->cosearch->type("application/json");
    my $res = $self->cosearch->get("/".$self->cosearch->{database}."/_local/".$self->localdocument,{},{deserializer => 'application/json'});
    if ($res->code == 200) {
...
sub putSince {
    ...
    $self->cosearch->type("application/json");
    my $res = $self->cosearch->put("/".$self->cosearch->{database}."/_local/".$self->localdocument,$newdoc);
    if ($res->code != 201 && $res->code != 200) {
        warn "_local/".$self->localdocument." PUT return code: " . $res->code . "\n";
    } else {
