...
sub update_couch {
    ...
    my $res = $dbo->get(
        "/".$dbo->{database}.
        "/_all_docs?key=\"".$self->aip."\"",{}, {deserializer => 'application/json'});
    if ($res->code == 200) {
        if ($res->data->{rows} && $res->data->{rows}[0]->{key}) {
            ...
            if (exists $docs->{$docid}) {
                ...
                warn "$docid has revision but not doc(".$dbo->{database}.")\n";
            }
        }
    } else {
    ...
    my $res = $dbo->get(
        "/".$dbo->{database}.
        "/_all_docs?startkey=\"".$self->aip.".\"&endkey=\"".
        $self->aip.".\ufff0\"",{}, {deserializer => 'application/json'});
    if ($res->code == 200) {
    ...
    if ($res->code == 201) {
        ...
        if (exists $data[0]->{id}) {
            foreach my $thisdoc (@data) {
                ...
                if (! $thisdoc->{ok} ) {
                    warn $thisdoc->{id}." was not indicated OK update_couch (".$dbo->{database}.")\n";
                    $self->{ustatus} = 0;
                }
            }
        }
    } else {
        die "update_couch (".$dbo->{database}.") POST return code: ".$res->code."\n";
    }
}
...
sub process_hammer {
    ...
    my $hammerdata = $self->internalmeta->get_aip($self->aip."/hammer.json");
    ...
    for my $i (0 .. $#$hammerdata) {
        my $doc=$hammerdata->[$i];
        my $key = $doc->{'key'} || die "Key missing from document in Hammer.json";
        ...
        foreach my $cf ("key","type","label",
                        "pkey","seq","lang","media","identifier",
                        "canonicalUri","canonicalMaster","canonicalMasterMime",
        ...
        if (keys %docfields) {
            warn "Unused Hammer fields in $key: " . join (",",keys %docfields) ."\n";
            print "Unused Hammer fields in $key: " . join (",",keys %docfields)."\n";
        }
    }
}
...
sub process_externalmetaHP {
    ...
    foreach my $seq (keys $emHP) {
        ...
        if (defined $self->searchdoc->{$pageid}) {
            ...
            # In parent as well..
            foreach my $cf ("tag","tagPerson","tagName","tagPlace",
                            "tagDate","tagNotebook","tagDescription") {
                if(exists $tags->{$cf}) {
                    if (ref($tags->{$cf} ne "ARRAY")) {
                        die "externalmetaHP tag $cf for page $pageid not array\n";
                    }

                    mergemulti($self->searchdoc->{$pageid},$cf,$tags->{$cf});
                    mergemulti($self->presentdoc->{$pageid},$cf,$tags->{$cf});
                    mergemulti($self->searchdoc->{$self->aip},$cf,$tags->{$cf});
                    mergemulti($self->presentdoc->{$self->aip},$cf,$tags->{$cf});
                }
                delete $tagfields{$cf};
            }

            # Set flag in item to indicate this component has tags
            $self->presentdoc->{$self->aip}->{'components'}->{$pageid}->{'hasTags'}=JSON::true;
            # Set flag in item to indicate some component has tags
            $self->presentdoc->{$self->aip}->{'hasTags'}=JSON::true;

            if (keys %tagfields) {
                warn "Unused externalmetaHP fields in $pageid: " . join (",",keys %tagfields) ."\n";
            }
        } else {
            die "externalmetaHP sequence $seq doesn't exist in".$self->aip."\n";
        }
    }
}
...
sub process_issue {
    ...
    my $res = $self->internalmeta->post("/".$self->internalmeta->{database}."/_design/tdr/_update/parent/$parent", {}, {deserializer => 'application/json'});
    if ($res->code != 201 && $res->code != 200) {
        die "_update/parent/$parent POST return code: " . $res->code . "\n";
    }
    if ($res->data->{return} ne 'updated') {
        die "_update/parent/$parent POST function returned: " . $res->data->{return} . "\n";        
    }
...
sub process_series {
    ...
    my $res = $self->internalmeta->get("/".$self->internalmeta->{database}."/_design/tdr/_view/issues?reduce=false&startkey=[\"".$self->aip."\"]&endkey=[\"".$self->aip."\",{}]", {}, {deserializer => 'application/json'});
    if ($res->code != 200) {
        die "_view/issues for " . $self->aip ." return code: " . $res->code . "\n";
    }
...
sub process_components {
    ...
    foreach my $thisdoc (keys $self->presentdoc) {
        ...
        $components->{$self->presentdoc->{$thisdoc}->{'key'}}->{'canonicalMaster'}=
            $self->presentdoc->{$thisdoc}->{'canonicalMaster'};
        if (exists $self->presentdoc->{$thisdoc}->{'canonicalMasterWidth'}) {
            $components->{$self->presentdoc->{$thisdoc}->{'key'}}->{'canonicalMasterWidth'}=
